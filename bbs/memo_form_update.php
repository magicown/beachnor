...
for ($i=0; $i<count($recv_list); $i++) {
    ...
    /*
    // 관리자가 아니면서
    // 가입된 회원이 아니거나 정보공개를 하지 않았거나 탈퇴한 회원이거나 차단된 회원에게 업무를 보내는것은 에러
    ...
    if (count($member_list['id'])) {
        ...
        if ($point) {
            if ($member['mb_point'] - $point < 0) {
                alert('보유하신 포인트('.number_format($member['mb_point']).'점)가 모자라서 업무를 보낼 수 없습니다.');
            }
        }
    }
}*/
...
for ($i=0; $i<count($member_list['id']); $i++) {
    ...
    // 업무 INSERT
    $sql = " insert into {$g5['memo_table']} ( me_id, me_recv_subjet, me_recv_mb_id, me_send_mb_id, me_send_datetime, me_memo ) values ( '$me_id', '$me_recv_subjet','$recv_mb_id', '{$member['mb_id']}', '".G5_TIME_YMDHIS."', '{$_POST['me_memo']}' ) ";
    sql_query($sql);
    ...
    if (!$is_admin) {
        insert_point($member['mb_id'], (int)$config['cf_memo_send_point'] * (-1), $recv_mb_nick.'('.$recv_mb_id.')님께 업무 발송', '@memo', $recv_mb_id, $me_id);
    }
}

if ($member_list) {
    $str_nick_list = implode(',', $member_list['nick']);
    alert($str_nick_list." 님께 업무를 전달하였습니다.", G5_HTTP_BBS_URL."/memo.php?kind=send", false);
} else {
    alert("받는 분 회원아이디 오류 같습니다.", G5_HTTP_BBS_URL."/memo_form.php", false);
}
...
@mkdir(G5_DATA_PATH.'/up_file', G5_DIR_PERMISSION);
@chmod(G5_DATA_PATH.'/up_file/', G5_DIR_PERMISSION);
