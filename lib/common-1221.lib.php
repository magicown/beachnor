...
function list_cate($name,$lx='')
{
  
	$sql = " select `list` from `cate` where `fenlei` = '$name' ";
	$cate = sql_fetch($sql);
	$cate_ex = explode("\n",$cate['list']);
	/*if(!$lx){
		sort($cate_ex);
...
function edt_cate($cate){
	
	global $is_admin;
	
	//if($is_admin){
	echo '<a href="javascript:;" onClick="pop_out(80);mo_cate_in(\''.$cate.'\');"><img src="/img/icon_xiu.png" class="xiu"></a>';	
	//}
	
}


function level_txt($txt){
