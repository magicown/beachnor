    ...
    <div class="bo_fx" id="bo_fx" style="display:block !important">
      ...
      <form name="fsearch" method="get">
        ...
        <select name="s_content" >
            <option value="" <?=$s_content==''?' selected="selected"':''?>>선택하세요</option>
            <option value="남" <?=$s_content=='남'?' selected="selected"':''?>>남</option>
            <option value="여" <?=$s_content=='여'?' selected="selected"':''?>>여</option>
        </select>
        ...
        지역:
        <input type="text" name="s_5" value="<?=$s_5?>" size="15">
        
        업무:
        <input type="text" name="s_6" value="<?=$s_6?>" size="15">
        ...
        파견종료일 : 
        <input type="text" name="sdate2" id="datetime3" style="width:75px;" value="<?=$sdate2?>">
        -
        <input type="text" name="edate2" id="datetime4" style="width:75px;" value="<?=$edate2?>">
...
<li><input type="submit" name="btn_submit" value="메일보내기" onclick="document.pressed=this.value" class="button white  small"></li>
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="col">업무</th>
            <th scope="col">파견일</th>
            <th scope="col">파견종료일</th>
            <th scope="col">상태</th>
            
            <?
            if($typ!=1 && $member['mb_level']!="8" && $member['mb_level']!="7"){
			?>
        ...
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?> out_<?=$sty_1?>">
			...
			?>
                  <td><a href="/tel_form.php?code=<?=urlencode($list[$i]['wr_link2'])?>" target="_blank"  class="button white  small win_memo">문자보내기</a></td>
                   <td><a href="/email_form.php?mb_id=<?=$list[$i]['wr_12']?>"  class="button white  small win_memo" target="_blank" id="ol_after_memo">메일보내기</a></td>
                    <?  }?>
                    
                    
                    
                  <td> <a href="<?php echo $list[$i]['href'] ?>"><strong style="color:#FF575A">보기</strong></a>
    ...
    <div class="bo_fx" style="display:block">
        ...
        <ul class="btn_bo_user">
            ...
            <?php if ($write_href&&$member['mb_level']!=7&&$member['mb_level']!=8) { ?><li><a href="/dex_a_1.php?sca=<?=$sca.$search_lk?>" class="button blue">엑셀다운</a></li><?php } ?>
            <?php if ($list_href&&$member['mb_level']!=7&&$member['mb_level']!=8) { ?><li><a href="<?php echo $list_href ?>" class="button black">목록</a></li><?php } ?>
            <?php if ($write_href&&$member['mb_level']!=8) { ?><li><a href="<?php echo $write_href ?>" class="button black">등록하기</a></li><?php } ?>
        </ul>
        <?php } ?>
    </div>
    <?php } ?>
