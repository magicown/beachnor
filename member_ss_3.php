...
$sql = " SELECT * FROM  `g5_member` WHERE (`mb_level`=5 || `mb_level`=6) and `mb_name` LIKE  '%".$_POST['mb_name']."%'";
$list = sql_query($sql);
...
<?
 for ($i=0; $row = sql_fetch_array($list); $i++) {
    ...
    <td><a href="javascript:;" onclick="find_zip('<?=$row['mb_name']?>', '<?=$row['mb_1']?>', '<?=$row['mb_birth']?>', '<?=$row['mb_hp']?>', '<?=$row['mb_email']?>', '<?=$row['mb_addr1']?> <?=$row['mb_addr2']?>', '<?=$row['mb_id']?>');"><?=$row['mb_name']?>(<?=$row['mb_id']?>)</a></td>
<? }?>
